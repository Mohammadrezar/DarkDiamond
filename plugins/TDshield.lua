...
if is_chat_msg(msg) or is_super_group(msg) then
	...
	if settings.strict then
		...
		if msg.text then -- msg.text checks
			...
			local is_link_msg = msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.text:match("[Tt].[Mm][Ee]/") or msg.text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
		...
		if msg.media then -- msg.media checks
			if msg.media.title then
				local is_link_title = msg.media.title:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/")or msg.media.title:match("[Tt].[Mm][Ee]/") or msg.media.title:match("[#@]") or msg.media.title:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
			...
			if msg.media.description then
				local is_link_desc = msg.media.description:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/")or msg.media.description:match("[Tt].[Mm][Ee]/") or msg.media.description:match("[#@]") or msg.media.description:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/")
			...
			if msg.media.caption then -- msg.media.caption checks
				local is_link_caption = msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/")or msg.media.caption:match("[Tt].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]/") or msg.media.caption:match("[#@]")
      ...
      if lock_strict == "no" then
        ...
        delete_msg(msg.id, ok_cb, true)
					if link_flood == 'ban' then
					  ...
					  ban_user(msg.from.id, msg.to.id)
					elseif link_flood == 'kick' then
					  send_msg("channel#id"..msg.to.id, '<code> ارسال فوروارد مجاز نیست!(کاربر خاطی از گروه اخراج گردید) </code>\n <i> >مشخصات ارسال کننده فوروارد: </i> ('..USERNAME..')\n <b>(kicked from Supergroup)</b>',ok_cb,false)
				delete_msg(msg.id, ok_cb, false)
				delete_msg(msg.id, ok_cb, false)
				delete_msg(msg.id, ok_cb, false)
         kick_user(msg.from.id, msg.to.id)
