...
local group_porn_lock = data[tostring(target)]['settings']['lock_porn']
  ...
  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then
   return '》ارسال عکس ۱۸+ در گروه از قبل ممنوع بود🔒 \n》توسط: @'..msg.from.username..' '
    else
    return '<i>》Send porn photo is already locked🔒 \n》Order By: </i>@'..msg.from.username..' '
  end
  end
    ...
    local group_lang = redis:hget(hash,'lang')
    if group_lang then
    return '》ارسال عکس  ۱۸+‌در گروه ممنوع شد🔒 \n》توسط: @'..msg.from.username..' '
    else 
    return '<i>》Send porn photo has been locked🔒 \n》Order By: </i>@'..msg.from.username..' '
  end
end
...
local function unlock_group_porn(msg, data, target)
  ...
  local hash = 'group:'..msg.to.id
    local group_lang = redis:hget(hash,'lang')
    if group_lang then
    return  '》ارسال عکس ۱۸+ در گروه آزاد بود🔓 \n》توسط: @'..msg.from.username..' '
   else
    return '<i>》send porn photo not locked🔓 \n》Order By: </i>@'..msg.from.username..' '
  end
end
    ...
    local group_lang = redis:hget(hash,'lang')
    if group_lang then
    return '》ارسال عکس ۱۸+ در گروه آزاد شد🔓 \n》توسط: '..msg.from.username..' '
    else
    return '<i>》send porn photo has been unlocked🔓 \n》Order By: </i>@'..msg.from.username..' '
  end
end
